...
@Getter
@ToString
public abstract class MyBaseTree<ID extends Comparable<ID>, E extends MyBaseTree> implements Serializable {
     ...
     * @return 返回树层次结构
     */
    public static <T extends MyBaseTree> List<T> build(List<T> baseTrees) {
     ...
     * @param <T>       泛型
     */
    private static <T extends MyBaseTree> void execute(MyBaseTree parent, List<T> baseTrees) {
        List<T> children = baseTrees.parallelStream().filter(b -> b.getParentId().compareTo(parent.value) == 0).collect(Collectors.toList());
        if (children.isEmpty()) {
            return;
        }
     ...
     * @param <T>        泛型
     */
    public static <T extends MyBaseTree> void recursionSort(List<T> trees, Comparator comparator) {
        if (Objects.isNull(trees) || trees.isEmpty()) {
            return;
        }
        trees.sort(comparator);
        for (MyBaseTree baseTree : trees) {
    ...
    public boolean equals(Object obj) {
        if (obj instanceof MyBaseTree) {
            MyBaseTree baseTree = (MyBaseTree) obj;
            if (baseTree.getValue() != null) {
                return baseTree.getValue().equals(getValue());
            }
        }
